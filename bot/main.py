...
async def on_ready():
    ...
    await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=data['status']))
...
async def newurl(ctx, handle, url):
    ...
    try:
        ...
        if statuscode == 400:
            await ctx.send("You're URL to shorten is bad. Note that the usage is: 'n.s {handle} {looong url here}''")
        if statuscode == 409:
